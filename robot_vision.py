...
class ArmComm:
    ...
    def pick(self, class_name, x_cm, y_cm, w, h):
        """
        Send a PICK command. class_name should be the actual waste class
        (e.g. "PLASTIC", "PAPER") or "GARBAGE" for non-recyclable items.
        Arduino handles the GARBAGE class by picking at (x_cm, y_cm) then
        dropping to the fixed bin position automatically.
        """
...
def detect_best(frame, model):
    ...
    for xyxy, conf, c in zip(
        ...
        if conf_val > best_conf:
            ...
            best = {
                "class_id":   int(c),
                "class_name": arm_class,       # "garbage" for non-recyclables
                "display_name": name,          # original class for logging/HUD
                "recycle":    recycle,
...
def main():
    ...
    while True:
        ...
        # --- Feedback loop finished: pick confirmed ---
            ...
            if det:
                ...
                if confirm >= DETECT_CONFIRM_FRAMES:
                    ...
                    if not in_workspace(x_cm, y_cm):
                        ...
                        # Open gripper before moving to object
                        arm.grip()
                        ...
                        if arm.pick(det["class_name"], x_cm, y_cm, w, h):
                            if arm.wait_done(15):
                                if det["class_name"] != "garbage":
                                    # Recyclable: close gripper after arm reaches object
                                    arm.release()
                                    if not arm.wait_done(2):
                                        logger.warning("Gripper close timed out")

                                # Start feedback loop to confirm object is gripped
                                busy = True
