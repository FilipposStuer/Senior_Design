 ...
 *
 * Point 4 fix applied:
 *   - Serial protocol replies are made more consistent
 *   - success replies use both Serial and Serial1 where appropriate
 *   - error replies are explicit and machine-detectable
 *
 * No other logic was intentionally changed beyond preserving your already-made edits
 * and fixing syntax/compile issues in the pasted version.
 */
...
void toIKSpace(float rx, float ry, float &ikx, float &iky) {
  ...
  ikx = sqrtf((rx + 3) * (rx + 3) + ry * ry) + 2.0f;
    // mirror around 7.5 if original rx is close
  iky = 3.0f;
 // default floor height; callers may override
}
...
void printHelp() {
  ...
  Serial.println("    PLASTIC/METAL/CARDBOARD -> pick at coords, drop to class bin, HOME");
...
void processCommand(String raw) {
  ...
  if (upper.startsWith("MOVE")) {
    ...
    Serial.print(sqrtf(rx * rx + ry * ry));
    Serial.println(" in");

    Serial.print("IK space  (");
    Serial.print(ikx);
    Serial.print(", ");
    Serial.print(iky);
    Serial.println(")");
  ...
  // ── PICK (from vision) ──
    ...
    if (firstSpace == -1) {
      replyError("Invalid PICK format");
      return;
    }
    ...
    // ── PLASTIC / METAL / CARDBOARD: pick then drop to bin ──
    ...
    // ── All other classes (biodegradable, glass, paper): ignore ──
    Serial.print("Ignoring class: ");
  ...
  // ── UNKNOWN ──
    ...
    replyError("Unknown command \"" + raw + "\" -- type HELP");
  }
}
