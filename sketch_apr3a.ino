 ...
 *
 * MODIFICATION: Gripper opens at PICK command start and closes only
...
void processCommand(String raw) {
  ...
  // ── PICK (from vision) ──
    ...
    // --- NEW: Open gripper before moving to the object ---
    Serial.println("Opening gripper...");
    int openAngles[6];
    memcpy(openAngles, currentAngles, sizeof(openAngles));
    openAngles[5] = GRIPPER_OPEN;
    moveServosSmooth(openAngles, 10);
    delay(300);   // allow gripper to fully open
    // --------------------------------------------------
    ...
    // --- NEW: Close gripper only after reaching the pick position ---
    Serial.println("Closing gripper...");
    int closeAngles[6];
    memcpy(closeAngles, currentAngles, sizeof(closeAngles));
    closeAngles[5] = GRIPPER_CLOSED;
    moveServosSmooth(closeAngles, 10);
    delay(300);
    // ---------------------------------------------------------------
  }
